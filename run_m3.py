...
from exp.exp_forecast_post_risk import Exp_Forecast_Post_Risk
...
def main():
    parser = argparse.ArgumentParser(description="M3: Forecast-only + Postprocessed Risk")

    parser.add_argument("--task_name", type=str, default="forecast_post_risk", choices=["forecast_post_risk"])
    ...
    parser.add_argument(
        "--forecast_ckpt_setting",
        type=str,
        default=None,
        help="Optional existing forecast-only setting dir to reuse during M3 testing.",
    )
    ...
    parser.add_argument("--label_mode", type=str, default="generated", choices=["generated"])
    ...
    Exp = Exp_Forecast_Post_Risk

    if args.is_training:
        for ii in range(args.itr):
            setting = (
                f"{args.task_name}_{args.model}_{args.features}_sl{args.seq_len}_pl{args.pred_len}_"
                f"nc{args.num_classes}_bs{args.batch_size}_lr{args.learning_rate}_{ii}"
            )
            exp = Exp(args)
