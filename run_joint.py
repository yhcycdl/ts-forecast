...
from exp.exp_joint_forecast_risk import Exp_Joint_Forecast_Risk
...
def main():
    parser = argparse.ArgumentParser(description="Joint Forecast + Risk Classification")

    parser.add_argument("--task_name", type=str, default="joint_forecast_risk", choices=["joint_forecast_risk"])
    ...
    parser.add_argument("--joint_forecast_weight", type=float, default=1.0)
    parser.add_argument("--joint_cls_weight", type=float, default=1.0)
    parser.add_argument("--joint_future_pool_bins", type=int, default=16)
    parser.add_argument("--joint_use_future_band_feature", type=int, default=1)
    parser.add_argument(
        "--joint_selection_metric",
        type=str,
        default="joint_loss",
        choices=["joint_loss", "forecast_loss", "forecast_mse", "cls_loss", "f1_macro", "balanced_accuracy", "bal_acc", "accuracy", "auprc", "auroc"],
    )
    parser.add_argument(
        "--joint_test_checkpoint",
        type=str,
        default="selected",
        choices=["selected", "best_joint_loss", "best_f1_macro", "best_forecast_mse"],
    )
    ...
    Exp = Exp_Joint_Forecast_Risk

    if args.is_training:
        for ii in range(args.itr):
            setting = (
                f"{args.task_name}_{args.model}_{args.features}_sl{args.seq_len}_pl{args.pred_len}_"
                f"nc{args.num_classes}_jf{args.joint_forecast_weight}_jc{args.joint_cls_weight}_"
