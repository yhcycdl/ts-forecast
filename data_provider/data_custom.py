...
class RiskWindowDataset(Dataset):
    ...
    输出:
      ...
      y: int64
    """
    def __init__(self, series, labels, seq_len, pred_len, stride=1, start_indices=None, label_config=None, window_stats=None):
        if series.ndim == 1:
            series = series[:, None]
        ...
        if start_indices is None:
            ...
            if expected <= 0:
                raise ValueError(
                    "Not enough timesteps to build any risk-classification window. "
                    f"Need at least seq_len + pred_len = {self.seq_len + self.pred_len}, got {self.series.shape[0]}."
                )
            ...
            if start_indices.size == 0:
                raise ValueError("No risk-classification windows found for this split.")
