...
class Model(nn.Module):
    ...
    def forecast(self, x):
        ...
        if self.out_channels <= self.in_channels:
            last_point = x[:, :self.out_channels, -1].unsqueeze(1)
        else:
            last_point = x[:, :1, -1].unsqueeze(1).expand(-1, 1, self.out_channels)
        ...
        base = last_point.expand(-1, self.pred_len, -1)  # (B,P,Cout)
        return base + delta
