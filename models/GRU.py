...
class Model(nn.Module):
    ...
    def _to_bcl(self, x):
        return to_bcl(x, self.seq_len, self.in_channels)

    def _encode(self, x):
        x = self._to_bcl(x)
        if self.out_channels <= self.in_channels:
            last_value = x[:, :self.out_channels, -1]
        else:
            last_value = x[:, :1, -1].expand(-1, self.out_channels)
    ...
    def forecast(self, x):
        ...
        return delta + last_value.unsqueeze(1)
